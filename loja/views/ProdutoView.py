...
def list_produto_view(request, id=None):
    ...
    if produto is not None:
        ...
        produtos = produtos.filter(Produto__contains=produto ) #Para consultar strings parciais 
    ...
    if id is not None:
        ...
        print(produtos) # Adicione para definir o contexto e carregar o template
...
def edit_produto_postback(request, id=None):
    if request.method == 'POST': #Salva dados editados
    ...
    return redirect("/produto") # Processa o post back gerado pela action

def details_produto_view(request, id=None):# Processa o evento GET gerado pela action
    produtos = Produto.objects.all()
    if id is not None:
        produtos = produtos.filter(id=id)
    produto = produtos.first()
...
def delete_produto_view(request, id=None):# Processa o evento GET gerado pela action
    produtos = Produto.objects.all()
    if id is not None:
        produtos = produtos.filter(id=id)
    produto = produtos.first()
...
def delete_produto_postback(request, id=None):# Processa o post back gerado pela action
    if request.method == 'POST':# Salva dados editados
        id = request.POST.get("id")
        produto = request.POST.get("Produto")
